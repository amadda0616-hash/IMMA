...
> ★★★ **버전 A 종료 (2026-05-06)** — 한글 도면 → JSON 출력 (Stage 4) ★ PASS.
> ★ **D-058 색상 정규화 wrapper 자동 통합 완료** — 다크 모드 / 청색 / 녹색 도면 자동 처리.
> 버전 B (1280+ ko/ja, YOLO26, 코랩 Pro+, 멀티 OCR, 색상 정규화 모델 통합) 준비 중 — 데이터셋 분석 후 시작.
>
> **버전 A 박제** (★ 모든 .md 파일 `*_ver.A.md` rename 완료):
> - 인수인계: [`PROJECT_HANDOFF_ver.A.md`](./PROJECT_HANDOFF_ver.A.md)
> - 한계: [`docs/KNOWN_LIMITATIONS_ver.A.md`](./docs/KNOWN_LIMITATIONS_ver.A.md)
> - Quick Start: [`instruction_ver.A.md`](./instruction_ver.A.md)
> - 종료 marker: `history_ver.A.md §A.12.20` (★ 진짜 final) + `PROJECT_HANDOFF_ver.A.md §10.X`
...
## ★ 버전 A 최종 결과 (2026-05-06)

| Stage | 결과 | 평가 |
|---|---|---|
| Stage 1 (yolo_det.pt V.A) | mAP@50 0.94 | ✅ PASS |
| Stage 2 (yolo_obb 5-fold ensemble) | D-023 PASS | ✅ |
| Stage 3-A (PaddleOCR-VL-1.5 subprocess) | char acc 0.69 | ⚠️ 부분 PASS |
| Stage 3-N (Donut Numerical fine-tune) | numerical 3.4% / hallucination 72% | ⚠️ baseline only |
| Stage 4 (JSON Merger, pipeline.py) | 5/5 schema PASS | ✅ |
| **★ 한글 도면 e2e** | ko_drawing.json 정상 생성 (TB + Notes 5 lines) | ✅ |

---

## ★ Quick Start — 임의 도면 → JSON 출력 (★ 1줄)

```bash
# wrapper 스크립트 (★ Final.1 + D-058 자동 색상 정규화 통합)
chmod +x run_pipeline_ver.A.sh
./run_pipeline_ver.A.sh dataset/<sample>.jpg ko outputs/

# 또는 직접 pipeline.py 호출 (★ 색상 정규화 수동 처리 필요)
PYTHONPATH=. python src/pipeline.py run \
    --image dataset/<sample>.jpg \
    --donut-num checkpoints/donut_numerical/final \
    --device cuda:0 --language ko \
    --out outputs/<sample>.json --keep-tmp
```

**예상 시간**: ~5~10분 / 도면 1장 (Stage 1 ~50s + Stage 2 ~5s + Stage 3-A ~50s × N region + Stage 3-N ~180s)

★★★ **자동 색상 정규화 (D-058)** — wrapper 가 자동 처리:
- ★ 다크 모드 (검은 배경 + 녹/시안 선) → 색상 반전 + grayscale → V.A 검출 정상
- ★ 청색 도면 (Blueprint, 청색 배경) → 반전 + grayscale
- ★ 녹색 / 강한 색상 도면 → grayscale 만
- ★ 표준 (흰 배경 + 검은 선) → 변경 X
- ★ wedge.jpg (CAD 다크 모드) PASS 검증 — TB 1 / Notes 7 / Views 4 / 34 ann

수동 색상 정규화 (★ 옵션):
```bash
# 자동 분석 (배경 luminance + dominant color)
python src/preprocess_color_normalize.py --input <img> --analyze-only

# 강제 반전
python src/preprocess_color_normalize.py --input <img> --output <img_norm> --force-invert

# 강제 grayscale
python src/preprocess_color_normalize.py --input <img> --output <img_norm> --force-grayscale
```

**기대 결과**:
- `outputs/<sample>.json` (★ HANDOFF §5.5 schema, ~60KB)
- `outputs/_pipeline_tmp/` (디버깅용 — `--keep-tmp` 옵션)

**JSON 구조** (요약):
```json
{
  "drawing_id": "...",
  "title_block": {"_raw_text": "..."},
  "notes": ["...", ...],
  "views": [{"view_id": "view_0", "annotations": [...]}],
  "meta": {"timing_seconds": {...}}
}
```

★ **한글 도면 정상 처리 검증** (Phase 17 e2e batch — `ko_drawing.json`).
...
> 기준: 2026-04-27 (V6 완료 시점). 정확한 최신은 `PROJECT_HANDOFF_ver.A.md` §10 참조.
...
- Train/val split 시 `sklearn.model_selection.GroupShuffleSplit` 사용 권장 (PROJECT_HANDOFF_ver.A.md §11 D-024 참조)

### 완료 (DONE)

| Step | 모듈 / 산출물 | 비고 |
|---|---|---|
| 0   | `PIPELINE_ver.A.md` | 논문 분석 + 초기 파이프라인 정리 |
| 0.5 | `PROJECT_HANDOFF_ver.A.md` | 인수인계 사양서 (§0~§14, 의사결정 D-001~D-018) |
| 1   | `requirements.txt`, `configs/*.yaml`, `.gitignore`, `README_ver.A.md` | 환경 + 디렉터리 골격 |
...
상세: [`history_ver.A.md`](./history_ver.A.md) §A — Version A 전체 기록.
...
상세: [`history_ver.A.md`](./history_ver.A.md) §A.6.3.
...
| **Day 4 DONE** (2026-05-05 ~ 06) | Phase 15b/16a/16b/16c/15c 완료 + Phase 17 진입 ★ | **(이전)** Phase 15b 1~4차 → D-045~D-048 + ja_drawing 110 region 분리. **Phase 16a/b**: 11,470 region + auto_fill 50.4% + Phase 16b 학습 성공 (eval_loss 0.9581, 6h 23분) + D-049~D-054. **(★ Day 5 신규)** **Phase 16c V6 검증** (D-055 baseline 검증: numerical 3.4% / hallucination 72% — D-051 가설 검증 완료) + **★ Phase 15c 완료 (D-056)** — pipeline.py 의 PaddleOCR-VL backend 통합 (subprocess wrapper + 4 fix: stderr redirect / atexit / images_kwargs / 출력 키 mismatch) + **Phase 17 smoke test PASS** (Stage 1+2+3-A+3-N 통합 638s, title_block 채움). **★ 다음 (Day 5 후반)**: 5장 batch + V7 검증 + multi-TB merge 정책 후속 + Phase 18 검수 도구. |
...
```
Drawing/
├── README_ver.A.md                         ← 본 파일
├── PROJECT_HANDOFF_ver.A.md                ← ★ 인수인계/사양서 (필독)
├── PIPELINE_ver.A.md
├── MANUAL_ver.A.md                         ← 단계별 작업 가이드 (사용자 친화)
├── label_manual_ver.A.md                   ← 라벨링 매뉴얼 (Roboflow + CVAT)
├── history_ver.A.md                        ← ★ 학습 이력 (Version A, B, C ...)
...
상세: [`docs/modules/stage3_paddleocr_install_check.md`](./docs/modules/stage3_paddleocr_install_check.md) / [`docs/PHASE15_CHECKLIST_ver.A.md`](./docs/PHASE15_CHECKLIST_ver.A.md).
...
| 파일 | 역할 |
|---|---|
| [`PROJECT_HANDOFF_ver.A.md`](./PROJECT_HANDOFF_ver.A.md) | 인수인계 사양서 (자기완결형). §0 체크리스트, §3 아키텍처, §5 JSON 스키마, §10 진행현황(LIVE), §11 의사결정(D-001~D-018), §13 새 AI 부트스트랩 프롬프트 |
| [`PIPELINE_ver.A.md`](./PIPELINE_ver.A.md) | 초기 파이프라인 메모 |
| [`README_ver.A.md`](./README_ver.A.md) | 본 파일 |
...
| [`MANUAL_ver.A.md`](./MANUAL_ver.A.md) | 처음부터 끝까지 작업 매뉴얼 (uv 환경 + 라벨링 + 학습 + 검증) |
...
상세는 [`PROJECT_HANDOFF_ver.A.md`](./PROJECT_HANDOFF_ver.A.md) §11. 주요 항목만 (D-001 ~ D-034):
...
**최신 학습 결과**: [`history_ver.A.md`](./history_ver.A.md) §A — Version A (Stage 1 seed 100장, mAP 0.9364)
